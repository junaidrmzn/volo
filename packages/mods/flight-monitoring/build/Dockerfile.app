...
WORKDIR /voloiq/packages/mods/flight-monitoring
RUN yarn build

FROM nginx:stable-alpine as final

COPY --from=build /voloiq/packages/mods/flight-monitoring/dist /usr/share/nginx/html
COPY --from=build /voloiq/packages/mods/flight-monitoring/config/nginx.conf /etc/nginx/conf.d/default.conf
...
COPY ./packages/mods/flight-monitoring/build/start-nginx.sh /usr/bin/start-nginx.sh
