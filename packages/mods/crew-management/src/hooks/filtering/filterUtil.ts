...
export const createApiFilterString = <GetDTO>(filterSet?: FilterSet<GetDTO>): string => {
    ...
    for (const filter of filterSet.filters) {
        ...
        // @ts-ignore
        if (propertyName !== "homeBase" && propertyName !== "roleAssignments") {
...
export const extractRegionFilter = (filterString?: FilterSet<Event>) => {
    const regionFilter = filterString?.filters.find((filter) => filter.propertyName === "homeBase");
    const rolesFilter = filterString?.filters.find((filter) => filter.propertyName === "roleAssignments");

    return {
        ...(rolesFilter && {
            roles:
                rolesFilter.type === "multiSelect"
                    ? rolesFilter.values.map((option) => option.value).join(",")
                    : undefined,
        }),
