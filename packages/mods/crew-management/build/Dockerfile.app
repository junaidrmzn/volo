...
WORKDIR /voloiq/packages/mods/crew-management
RUN yarn build

FROM nginx:stable-alpine as final

COPY --from=build /voloiq/packages/mods/crew-management/dist /usr/share/nginx/html
COPY --from=build /voloiq/packages/mods/crew-management/config/nginx.conf /etc/nginx/conf.d/default.conf
...
COPY ./packages/mods/crew-management/build/start-nginx.sh /usr/bin/start-nginx.sh
