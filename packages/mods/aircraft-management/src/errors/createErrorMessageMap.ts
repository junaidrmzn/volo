        ...
        match(detailedError)
            .when(isMinReferencedAttributeValueError, (error) => {
                ...
                if (isFieldName(referencedAttribute)) {
                    errorMessageMap[attribute] = t(
                        "aircraft-management.errorMessages.minReferencedAttributeValueError",
                        {
            ...
            .when(isListEmptyError, () => {
                errorMessageMap[attribute] = t("aircraft-management.errorMessages.listEmptyError");
            })
            .when(isMaxCharacterError, (error) => {
                const { maxCharacter } = error;
                errorMessageMap[attribute] = t("aircraft-management.errorMessages.maxCharacterError", {
                    maxCharacter,
                });
            })
            .when(isMaxReferencedAttributeValueError, (error) => {
                ...
                if (isFieldName(referencedAttribute)) {
                    errorMessageMap[attribute] = t(
                        "aircraft-management.errorMessages.maxReferencedAttributeValueError",
                        {
                            referencedAttributeLabel: getFieldLabel(schema, referencedAttribute),
            ...
            .when(isMaxValueError, (error) => {
                ...
                errorMessageMap[attribute] = t("aircraft-management.errorMessages.maxValueError", {
                    maxValue: formatValue(maxValue),
                });
            })
            .when(isMinCharacterError, (error) => {
                const { minCharacter } = error;
                errorMessageMap[attribute] = t("aircraft-management.errorMessages.minCharacterError", {
                    minCharacter,
                });
            })
            .when(isMinValueError, (error) => {
                const { minValue } = error;
                errorMessageMap[attribute] = t("aircraft-management.errorMessages.minValueError", {
                    minValue: formatValue(minValue),
                });
            })
            .when(isNumericOnlyError, () => {
                errorMessageMap[attribute] = t("aircraft-management.errorMessages.numericOnlyError");
            })
            .when(isReferentialValueDependencyError, (error) => {
                const { referencedEntitiesAttribute, referencedEntitiesType } = error;
                errorMessageMap[attribute] = t("aircraft-management.errorMessages.referentialValueDependencyError", {
                    referencedEntitiesType,
                    referencedEntitiesAttribute,
                });
            })
            .when(isValueTakenError, () => {
                errorMessageMap[attribute] = t("aircraft-management.errorMessages.valueTakenError");
            })
            .when(isValidationError, () => {
                errorMessageMap[attribute] = t("aircraft-management.errorMessages.validationFailedError");
            })
            .when(isMaxForeignReferencedAttributeValueError, (error) => {
                ...
                errorMessageMap[attribute] = t(
                    "aircraft-management.errorMessages.maxForeignReferencedAttributeValueError",
                    {
            ...
            .when(isMinForeignReferencedAttributeValueError, (error) => {
                ...
                errorMessageMap[attribute] = t(
                    "aircraft-management.errorMessages.minForeignReferencedAttributeValueError",
                    {
                        referencedEntityAttribute: splitCamelCase(referencedEntityAttribute),
                        referencedEntityType: splitCamelCase(referencedEntityType),
