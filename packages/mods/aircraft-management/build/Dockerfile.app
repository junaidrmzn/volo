...
WORKDIR /voloiq/packages/mods/aircraft-management
RUN yarn build

FROM nginx:stable-alpine as final

COPY --from=build /voloiq/packages/mods/aircraft-management/dist /usr/share/nginx/html
COPY --from=build /voloiq/packages/mods/aircraft-management/config/nginx.conf /etc/nginx/conf.d/default.conf
...
COPY ./packages/mods/aircraft-management/build/start-nginx.sh /usr/bin/start-nginx.sh
