...
WORKDIR /voloiq/packages/mods/flight-planning
RUN yarn build

FROM nginx:stable-alpine as final

COPY --from=build /voloiq/packages/mods/flight-planning/dist /usr/share/nginx/html
COPY --from=build /voloiq/packages/mods/flight-planning/config/nginx.conf /etc/nginx/conf.d/default.conf
...
COPY ./packages/mods/flight-planning/build/start-nginx.sh /usr/bin/start-nginx.sh
