...
WORKDIR /voloiq/packages/mods/network-scheduling
RUN yarn build

FROM nginx:stable-alpine as final

COPY --from=build /voloiq/packages/mods/network-scheduling/dist /usr/share/nginx/html
COPY --from=build /voloiq/packages/mods/network-scheduling/config/nginx.conf /etc/nginx/conf.d/default.conf
...
COPY ./packages/mods/network-scheduling/build/start-nginx.sh /usr/bin/start-nginx.sh
