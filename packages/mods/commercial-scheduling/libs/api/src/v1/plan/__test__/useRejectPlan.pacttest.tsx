...
import { useRejectPlan } from "../useRejectPlan";
...
pactWith({ consumer: CONSUMER_VOLOIQ_UI, provider: PROVIDER_COMMERCIAL_SCHEDULING_API }, (provider) => {
    test("will reject commercial plan", async () => {
        await provider.addInteraction({
            state: `reject commercial plan with id ${planId}`,
            uponReceiving: "request to reject commercial plan",
            withRequest: {
                path: `${PLAN_BASE_URL}/${planId}/reject`,
                method: "PUT",
            },
            willRespondWith: {
                status: 200,
        ...
        const { result, waitForNextUpdate } = renderHook(() => useRejectPlan(planId), {
