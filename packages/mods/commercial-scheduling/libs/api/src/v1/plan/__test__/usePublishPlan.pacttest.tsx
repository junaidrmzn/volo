...
import { usePublishPlan } from "../usePublishPlan";
...
pactWith({ consumer: CONSUMER_VOLOIQ_UI, provider: PROVIDER_COMMERCIAL_SCHEDULING_API }, (provider) => {
    test("will publish commercial plan", async () => {
        await provider.addInteraction({
            state: `publish commercial plan with id ${planId}`,
            uponReceiving: "request to publish a commercial plan",
            withRequest: {
                path: `${PLAN_BASE_URL}/${planId}/publish`,
                method: "PUT",
            },
            willRespondWith: {
                status: 200,
        ...
        const { result, waitForNextUpdate } = renderHook(() => usePublishPlan(planId), {
