...
import { Aircraft, MissionConflict } from "@voloiq/network-schedule-management-api/v1";
import type { FilterProperty } from "@voloiq/resource-overview";
import { useMissionFilterTranslation } from "./translations/useMissionFilterTranslation";
...
export const useGetAllFilterProperties = () => {
    const { t } = useMissionFilterTranslation();
    const getAllFilterProperties = (props: UseGetAllFilterPropertiesProps): FilterProperty[] => {
        const { aircrafts, aircraftTypes, regions, vertiports } = props;
        return [
            // Flight number
            {
                type: "text",
            ...
            {
                type: "select",
                propertyName: "mission.scheduledDepartureVertiport.id",
            ...
            {
                type: "select",
                propertyName: "mission.scheduledArrivalVertiport.id",
                options: vertiports?.map((vertiport) => ({
                    label: vertiport.name,
                    value: vertiport.id,
                })),
            ...
            {
                type: "select",
                propertyName: "mission.scheduledDepartureVertiport.regionId",
            ...
            {
                type: "select",
                propertyName: "mission.scheduledArrivalVertiport.regionId",
                options: regions?.map((region) => ({
                    label: region.name,
                    value: region.id,
                })),
