...
import { useBulkEditImportantNotes } from "./useBulkEditImportantNotes";

const { like, term, eachLike } = Matchers;

const editImportantNoteRequest = (
    definitionId: string,
    procedureId: string,
...
const editImportantNoteResponse: ResponseOptions = {
    status: 200,
};
pactWith({ consumer: PACT_CONSUMER_FTD_UI, provider: PACT_PROVIDER_FTD_API }, (provider) => {
    test("bulk-updates important notes", async () => {
        const definitionId = "5bce990b-b76c-4241-95a1-674f9f2a62e3";
        const procedureId = "5bce990b-b76c-4241-95a1-674f9f2a62e3";
        const importantNoteId = "5bce990b-b76c-4241-95a1-674f9f2a62e3";
        await provider.addInteraction({
            ...
            uponReceiving: "a request for bulk-updating important notes",
            withRequest: editImportantNoteRequest(definitionId, procedureId, importantNoteId),
            willRespondWith: editImportantNoteResponse,
        });

        const { result, waitForNextUpdate } = renderHook(
            () => useBulkEditImportantNotes({ definitionId, procedureId }),
            {
        ...
        act(() => {
            result.current.bulkEditImportantNotes({
                data: [
                    {
                        id: importantNoteId,
                        title: "Hover Turn",
