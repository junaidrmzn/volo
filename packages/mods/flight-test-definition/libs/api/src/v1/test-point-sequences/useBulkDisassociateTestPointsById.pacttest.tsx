...
import { useBulkDisassociateTestPointsById } from "./useBulkDisassociateTestPointsById";

const { term, eachLike } = Matchers;

const bulkDisassociateTestPointsByIdRequest = (
...
const bulkDisassociateTestPointsByIdResponse: ResponseOptions = {
    status: 204,
};
...
pactWith.skip({ consumer: PACT_CONSUMER_FTD_UI, provider: PACT_PROVIDER_FTD_API }, (provider) => {
    test("bulk-deletes specific test point sequence test point associations", async () => {
        ...
        await provider.addInteraction({
            ...
            withRequest: bulkDisassociateTestPointsByIdRequest(flightTestOrderId, testPointSequenceId),
            willRespondWith: bulkDisassociateTestPointsByIdResponse,
        });

        const { result, waitForNextUpdate } = renderHook(
            () => useBulkDisassociateTestPointsById({ flightTestOrderId, testPointSequenceId }),
            {
        ...
        act(() => {
            result.current.bulkDisassociateTestPointsById({
