import type { ProcedurePatch, ProcedureRead } from "@voloiq-typescript-api/ftd-types";
...
export const useEditProcedure = (options: UseEditProcedureOptions) => {
    ...
    const editProcedure = async (config: EditProcedureRequest): Promise<EditProcedureResponse> => {
        const {
            ...
            ProcedurePatch
        >(`${baseUrl}/ftd/v2/definitions/${definitionId}/procedures/${procedureId}`, config?.data, {
            ...config,
