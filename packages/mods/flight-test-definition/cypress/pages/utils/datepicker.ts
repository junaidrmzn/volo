...
export const DateTimePicker = {
    ...
    selectDateTime: (date: Date) => {
        ...
        cy.get(`.flatpickr-calendar.open [aria-label="Minute"]`).type(String(finalDate.getMinutes()).padStart(2, "0"));
        DateTimePicker.selectDate(date);
        cy.get(`.flatpickr-calendar.open [aria-label="Minute"]`).type("{enter}");
    },
};
