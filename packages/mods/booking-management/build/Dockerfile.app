...
WORKDIR /voloiq/packages/mods/booking-management
RUN yarn build

FROM nginx:stable-alpine as final

COPY --from=build /voloiq/packages/mods/booking-management/dist /usr/share/nginx/html
COPY --from=build /voloiq/packages/mods/booking-management/config/nginx.conf /etc/nginx/conf.d/default.conf
...
COPY ./packages/mods/booking-management/build/start-nginx.sh /usr/bin/start-nginx.sh
