...
WORKDIR /voloiq/packages/mods/notifications
RUN yarn build

FROM nginx:stable-alpine as final

COPY --from=build /voloiq/packages/mods/notifications/dist /usr/share/nginx/html
COPY --from=build /voloiq/packages/mods/notifications/config/nginx.conf /etc/nginx/conf.d/default.conf
...
COPY ./packages/mods/notifications/build/start-nginx.sh /usr/bin/start-nginx.sh
