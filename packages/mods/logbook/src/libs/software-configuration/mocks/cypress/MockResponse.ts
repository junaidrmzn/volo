...
export type InterceptorOptions<Entety> = Pick<
    GenericStaticResponse<string, Entety>,
...
export const checkParamsMatches = (presentParams: CypressParameter, expectedParams?: CypressParameter) => {
    // eslint-disable-next-line unicorn/prefer-set-has
    const presentParamKeys = Object.keys(presentParams);

    if (presentParamKeys.length !== Object.keys(expectedParams || {}).length) {
        return false;
    }
    ...
    for (const expectedParamsKey in expectedParams) {
        if (
            ...
            !(
                ...
                presentParams[expectedParamsKey] === expectedParams[expectedParamsKey]
            )
        ) {
            return false;
        }
