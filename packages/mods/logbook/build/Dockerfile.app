...
WORKDIR /voloiq/packages/mods/logbook
RUN yarn build

FROM nginx:stable-alpine as final

COPY --from=build /voloiq/packages/mods/logbook/dist /usr/share/nginx/html
COPY --from=build /voloiq/packages/mods/logbook/config/nginx.conf /etc/nginx/conf.d/default.conf
...
COPY ./packages/mods/logbook/build/start-nginx.sh /usr/bin/start-nginx.sh
